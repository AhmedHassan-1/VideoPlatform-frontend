// src/hooks/useUpload.ts — qualities param removed (admin-controlled)
...
export function useUpload() {
  const [state, setState] = useState<UploadState>({
    uploading:false, progress:0, phase:'idle', error:null, uploadId:null, videoId:null,
  });
  ...
  const upload = useCallback(async (opts: UploadOptions) => {
    ...
    try {
      ...
      const initRes = await uploadApi.init({
        ...
        title:     opts.title,
        // No qualities — determined by admin allowedQualities on server side
      });
      ...
      for (let i = 0; i < effectiveTotalChunks; i++) {
        ...
        const buffer = await opts.file.slice(start, end).arrayBuffer();
  ...
  const cancel = useCallback(async () => {
    ...
    if (state.uploadId) await uploadApi.cancel(state.uploadId).catch(() => {});
