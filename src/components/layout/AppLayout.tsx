// src/components/layout/AppLayout.tsx — Updated: added Profile nav link, avatar links to profile
...
export default function AppLayout() {
  ...
  const storage = (profile as any)?.storage;
  const pct = storage?.unlimited ? 0 : Math.min(100, Math.round((storage?.used / storage?.limit) * 100)) || 0;

  async function handleLogout() {
    const result = await Swal.fire({ title:'Sign out?', icon:'question', showCancelButton:true, confirmButtonText:'Yes, sign out', cancelButtonText:'Stay', reverseButtons:true });
  ...
  return (
    <>
      {sidebarOpen && <div onClick={closeSidebar} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.6)', zIndex:99, backdropFilter:'blur(2px)' }} />}
      ...
      <div style={{ display:'flex', height:'100vh', background:'var(--bg-deep)', color:'var(--text)' }}>
        <aside className="sidebar" style={{ transform: sidebarOpen ? 'translateX(0)' : undefined }}>
          <div className="sidebar-logo" style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            ...
            <button onClick={closeSidebar} className="sidebar-close-btn" style={{ background:'none', border:'none', color:'var(--text-muted)', cursor:'pointer', padding:6, borderRadius:6, fontSize:18, lineHeight:1 }}>
          ...
          <nav className="sidebar-nav">
            {navItems.map(({ to, icon, label }, i) => (
              <NavLink key={to} to={to} end={to === '/dashboard'} onClick={closeSidebar}
            ...
            {user?.role === 'admin' && (
              <>
                ...
                <NavLink to="/admin" onClick={closeSidebar} className="nav-item" style={{ color:'var(--accent-2)' }}>
                  <i className="bi bi-shield-shaded" style={{ fontSize:15 }}></i><span>Admin Panel</span>
                </NavLink>
              </>
            )}
          </nav>
          ...
          <div style={{ padding:'12px 14px', borderTop:'1px solid var(--border)', display:'flex', alignItems:'center', gap:10 }}>
            <NavLink to="/profile" onClick={closeSidebar} title="View profile"
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
          <div className="mobile-topbar">
            <button onClick={() => setSidebarOpen(true)} style={{ background:'none', border:'none', color:'var(--text)', cursor:'pointer', padding:'8px', borderRadius:8, fontSize:18, lineHeight:1 }}>
